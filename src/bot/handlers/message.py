...
def _format_error_message(error: Exception | str) -> str:
    ...
    # Any future CodexError subtypes not explicitly handled above —
    ...
    # cli_integration.py and facade.py, NOT arbitrary user content.
    ...
    if "overloaded" in error_lower:
        return (
            "🏗️ <b>Codex is Overloaded</b>\n\n"
            "The Codex API is currently experiencing high demand.\n\n"
            "<b>What you can do:</b>\n"
    ...
    if "invalid api key" in error_lower or "authentication_error" in error_lower:
        return (
            ...
            "The API key used to connect to Codex is invalid or expired.\n\n"
            "<b>What you can do:</b>\n"
            "• Ask the administrator to verify the "
            "<code>OPENAI_API_KEY</code> setting\n"
            "• Check that the API key has not been revoked"
        )

    # Match known SDK prefixes: "Failed to connect to Codex: ..."
    # and "MCP server connection failed: ..."
    if error_lower.startswith("failed to connect to codex"):
        return (
            "🌐 <b>Connection Error</b>\n\n"
            f"Could not connect to Codex:\n"
            f"<code>{escape_html(error_str[:300])}</code>\n\n"
            "<b>What you can do:</b>\n"
    ...
    # Match known SDK prefix: "Codex not found. ..."
    if error_lower.startswith("codex not found"):
        return (
...
def _format_process_error(error_str: str) -> str:
    """Format a Codex process/SDK error with the actual details."""
    safe_error = escape_html(error_str)
    if len(safe_error) > 500:
        safe_error = safe_error[:500] + "..."

    return (
        f"❌ <b>Codex Process Error</b>\n\n"
        f"{safe_error}\n\n"
        "<b>What you can do:</b>\n"
        "• Try your request again\n"
    ...
    try:
        ...
        if conversation_enhancer and codex_response:
            try:
                ...
                conversation_enhancer.update_context(user_id, codex_response)
                conversation_context = conversation_enhancer.get_or_create_context(
                    user_id
                )

                # Check if we should show follow-up suggestions
                if conversation_enhancer.should_show_suggestions(codex_response):
...
async def handle_voice(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not voice_handler:
        if settings.voice_provider == "local":
            ...
            await update.message.reply_text(
                ...
                '<code>pip install "codex-telegram[voice]"</code>.',
                parse_mode="HTML",
            )
        return
    ...
    try:
        ...
        await progress_msg.edit_text(
            "🤖 Processing transcription with Codex...", parse_mode="HTML"
        )
