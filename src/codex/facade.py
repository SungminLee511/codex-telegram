...
from .exceptions import CodexTimeoutError
    ...
    async def run_command(
        ...
        try:
            ...
            try:
                ...
                # Wall-clock timeout — the session is still healthy on Codex's
                # side, just the user-configured timeout fired. Do NOT remove
                # the session; let the caller retry on the next message.
                logger.warning(
                    "Codex command timed out; preserving session for resume",
                    session_id=session.session_id,
                )
                raise
            except Exception as resume_error:
                # If resume failed (e.g., session expired/missing on Codex's side),
