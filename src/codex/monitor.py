"""Bash directory boundary enforcement for Codex tool calls."""

import shlex
from pathlib import Path
from typing import Optional, Set, Tuple

# Subdirectories under ~/.codex/ that Codex uses internally.
_CODEX_INTERNAL_SUBDIRS: Set[str] = {"plans", "todos", "settings.json"}
...
def _is_codex_internal_path(file_path: str) -> bool:
    """Check whether *file_path* points inside ``~/.codex/`` (allowed subdirs only)."""
    try:
        resolved = Path(file_path).resolve()
        home = Path.home().resolve()
        codex_dir = home / ".codex"

        # Path must be inside ~/.codex/
        try:
            rel = resolved.relative_to(codex_dir)
        except ValueError:
            return False

        # Must be in one of the known subdirectories (or a known file)
        top_part = rel.parts[0] if rel.parts else ""
        return top_part in _CODEX_INTERNAL_SUBDIRS
