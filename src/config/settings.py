...
from src.utils.constants import (
    DEFAULT_CODEX_MAX_COST_PER_REQUEST,
...
class Settings(BaseSettings):
    ...
    codex_binary_path: Optional[str] = Field(
        None, description="Path to Codex CLI binary (deprecated)"
    )
    codex_cli_path: Optional[str] = Field(
        None, description="Path to Codex CLI executable"
    )
    codex_model: Optional[str] = Field(
        None, description="Codex model to use (defaults to CLI default if unset)"
    )
    codex_reasoning_effort: Optional[Literal["minimal", "low", "medium", "high"]] = (
        Field(
            None,
            description=(
    ...
    codex_sandbox_mode: Optional[
        ...
        None,
        description=(
            "Codex CLI sandbox policy. Defaults to 'workspace-write' when "
            "sandbox_enabled, else 'danger-full-access'."
        ),
    )
    codex_skip_git_repo_check: bool = Field(
        True,
        description="Pass --skip-git-repo-check to allow running outside git repos",
    )
    codex_dangerously_bypass: bool = Field(
        False,
        description=(
            "Pass --dangerously-bypass-approvals-and-sandbox. EXTREMELY DANGEROUS — "
            "intended only for environments that are externally sandboxed."
        ),
    )
    codex_add_dirs: Optional[List[str]] = Field(
        default=None,
        description=(
            "Extra writable directories passed via --add-dir (comma-separated)."
        ),
    )
    codex_config_overrides: Optional[List[str]] = Field(
        default=None,
        description=(
            "Free-form Codex `-c key=value` overrides applied to every run "
            "(comma-separated entries)."
        ),
    )
    codex_max_turns: int = Field(
        DEFAULT_CODEX_MAX_TURNS, description="Max conversation turns"
    )
    codex_timeout_seconds: int = Field(
        DEFAULT_CODEX_TIMEOUT_SECONDS, description="Codex timeout"
    )
    codex_max_cost_per_user: float = Field(
        DEFAULT_CODEX_MAX_COST_PER_USER, description="Max cost per user"
    )
    codex_max_cost_per_request: float = Field(
        DEFAULT_CODEX_MAX_COST_PER_REQUEST,
    ...
    # Retry settings
    codex_retry_max_attempts: int = Field(
        DEFAULT_RETRY_MAX_ATTEMPTS,
        ge=0,
        description="Max retry attempts for transient SDK errors (0 = disabled)",
    )
    codex_retry_base_delay: float = Field(
        DEFAULT_RETRY_BASE_DELAY,
        ge=0,
        description=(
    ...
    codex_retry_backoff_factor: float = Field(
        DEFAULT_RETRY_BACKOFF_FACTOR,
        gt=0,
        description="Exponential backoff multiplier",
    )
    codex_retry_max_delay: float = Field(
        DEFAULT_RETRY_MAX_DELAY,
        ge=0,
        description=(
    ...
    @field_validator("codex_add_dirs", "codex_config_overrides", mode="before")
    @classmethod
    def parse_csv_list(cls, v: Any) -> Optional[List[str]]:
        """Parse comma-separated string lists."""
        if v is None:
            return None
        if isinstance(v, str):
            stripped = [item.strip() for item in v.split(",") if item.strip()]
    ...
    def openai_api_key_str(self) -> Optional[str]:
        """Get OpenAI API key as string (used for Codex auth + Whisper voice)."""
